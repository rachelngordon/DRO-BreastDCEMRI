...
def gen_dro(data: list, option: dict = None, num_frames: int = None, peak_window_size: int = 3):
    """
    ...
    if num_frames is None:
        ...
        # Override behavior: resample AIF, guaranteeing a window around the peak is included
        ...
        # --- NEW LOGIC TO PRESERVE PEAK SHAPE ---
        # 1. Find the N critical points around the original peak
        peak_idx = np.argmax(aif_original)
        half_window = peak_window_size // 2
        
        # Ensure window indices don't go out of bounds
        start_idx = max(0, peak_idx - half_window)
        end_idx = min(original_len - 1, peak_idx + half_window)
        
        t_critical = t_original[start_idx : end_idx + 1]
        
        # 2. Start with a base of evenly spaced time points
        t_base = np.linspace(0, SIMULATION_DURATION_S, num_frames)
        
        # 3. Remove points from the base set that are too close to the critical points we're adding
        t_final_list = list(t_base)
        for crit_pt in t_critical:
            # Find the closest point in the current list and remove it
            # This avoids clustering points and maintains the desired number of frames
            if t_final_list: # Check if list is not empty
                distances = [abs(pt - crit_pt) for pt in t_final_list]
                idx_to_remove = np.argmin(distances)
                del t_final_list[idx_to_remove]

        # 4. Create the final time vector by combining the lists and sorting
        t = np.sort(np.unique(np.concatenate((t_final_list, t_critical))))
        # --- END OF NEW LOGIC ---

        # 5. Resample the AIF using the new, peak-preserving time vector
